...
#define NX_JSON_CALLOC() (struct nx_json *)pet_malloc(sizeof(struct nx_json))
//(struct nx_json *)calloc(1, sizeof(struct nx_json))
#define NX_JSON_FREE(json) pet_free(json)
#endif
...
#define MAX_JSON_LEN (16 * 1024 * 1024)
...
__resize_srlzer(struct nx_json_serializer * srlzer)
{
	...
	tmp_ptr = pet_realloc(srlzer->str, srlzer->len * 2);
                    ...
                    struct nx_json_serializer * srlzer)
{
	...
	if (json->root == 0) {
		...
		if (json->parent->type != NX_JSON_ARRAY) {
	...
	switch (json->type) {
		...
		case NX_JSON_STRING: {
			char * escaped_str = escape_string(json->text_value);

			__srlzer_append(srlzer, "\"%s\"", escaped_str);

			NX_JSON_FREE(escaped_str);
			break;
		}
...
nx_json_serialize(struct nx_json * json)
{
	...
	srlzer.str = pet_malloc(sizeof(char) * 512);
	srlzer.len = 512;
	srlzer.off = 0;
	srlzer.lvl = 0;
...
__nx_json_fixup_strings(struct nx_json * obj)
{
	...
	if ((obj->key     != NULL) &&
		...
		obj->raw_key = pet_strndup(obj->key, MAX_JSON_LEN);
		obj->key     = obj->raw_key;
	}
	...
	if ((obj->type       == NX_JSON_STRING) &&
		...
		obj->raw_string = pet_strndup(obj->text_value, MAX_JSON_LEN);
            ...
            struct nx_json * val)
{
	...
	new_json->raw_key = pet_strndup(key, MAX_JSON_LEN);
	new_json->key     = new_json->raw_key;

	switch (val->type) {
		case NX_JSON_STRING:
			new_json->raw_string = pet_strndup(val->text_value, MAX_JSON_LEN);
            ...
            struct nx_json * val)
{
	...
	switch (val->type) {
		case NX_JSON_STRING:
			...
			if (tgt_obj->raw_string != NULL) {
				NX_JSON_FREE(tgt_obj->raw_string);
			}
	    
			tgt_obj->raw_string = pet_strndup(val->text_value, MAX_JSON_LEN);
...
escape_string(char * s)
{
	...
	new_str = pet_malloc(str_len + escaped_chars + 1);
	tmp_str = s;
	esc_str = new_str;
...
nx_json_parse(char * text)
{
	...
	raw_text = pet_strndup(text, MAX_JSON_LEN);
                 ...
                 struct nx_json * new_val)
{
	...
	switch (new_val->type) {
		case NX_JSON_STRING:
			if (tgt_item->raw_string != NULL) {
				NX_JSON_FREE(tgt_item->raw_string);
			}
	    
			tgt_item->raw_string = pet_strndup(new_val->text_value, MAX_JSON_LEN);
                 ...
                 struct nx_json * item)
{
	...
	switch (item->type) {
		case NX_JSON_STRING:
			new_item->raw_string = pet_strndup(item->text_value, MAX_JSON_LEN);
...
static int
nx_json_array_splice(struct nx_json * parent,
                     struct nx_json * new_json)
{
	assert(parent->type == NX_JSON_ARRAY);
